...
def _calc_ssim(image1: Image.Image, image2: Image.Image) -> float:
    """计算结构相似性指数（SSIM）
...
def _get_unsim_frames(
    ...
    while len(frames) > max_num_frames:
        ...
        for i in range(1, len(frames)):
            # 计算当前帧与最后一帧的SSIM
            ssim = _calc_ssim(remain_frames[-1], frames[i])
...
def gif2jpg(
    ...
    frames = _get_unsim_frames(image, ssim_threshold, max_frame_num)
