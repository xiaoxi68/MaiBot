...
class NormalChat:
    ...
    #         # 执行额外的动作（不影响回复生成）
    #         action_result = await self._handle_action(action_type, action_data, message_data, thinking_id)
    ...
    #     # 发送回复 (不再需要传入 chat)
    #     first_bot_msg = await add_messages_to_manager(message_data, reply_texts, thinking_id)
